 ...
 * Three控制台词汇库
 */
public class Practise_Three_E extends JFrame {
    /**
     * correct 获取总个数
     * remaining 获取正确个数
     * txtS1
    ...
    //记录当前随机数
    public Practise_Three_E() {
        z = (int) (Math.random() * Global_data.tc.size());
        //当前随机数
        ...
        txtS2.setColumns(10);
        txtS2.setText(Global_data.tc.get(z));
        //显示从集合中调用的内容内容
        ...
        btnNewButton_1.addMouseListener(new MouseAdapter() {
            ...
            public void mouseClicked(MouseEvent e) {
                Global_data.get1 = Global_data.tc.get(z);
                Global_data.get2 = Global_data.tv.get(Global_data.tc.get(z));
                //分别获把get的值赋值到到对应区域
                try {
                    Global_library.insert();
                    //执行插入数据
                    JOptionPane.showMessageDialog(null, "单词“" + Global_data.tc.get(z) + "”收录成功!");
                } catch (Exception ex) {
        ...
        btnNewButton_2.addMouseListener(new MouseAdapter() {
            @Override
            public void mouseClicked(MouseEvent e) {
                txtS3.setText(Global_data.tc.get(z));
                //题目
                txtS4.setText(Global_data.tv.get(Global_data.tc.get(z)));
                //答案
                correct--;
                //每次递减
                if (Global_data.tv.get(Global_data.tc.get(z)).equals(txtS1.getText())) {
                    remaining++;
                }//判断是否正确
                else {
                    if (Global_data.cka) {
