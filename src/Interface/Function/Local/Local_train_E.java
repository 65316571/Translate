 ...
 * User控制台词汇库
 */
public class Local_train_E extends JFrame {
    /**
     * correct 获取总个数
     * remaining 获取正确个数
     * txtS1
    ...
    public Local_train_E() {
        ...
        txtS2.setColumns(10);
        txtS2.setText(Global_data.uc.get(z));
        //显示从集合中调用的内容内容
        ...
        btnNewButton_2.addMouseListener(new MouseAdapter() {
            @Override
            public void mouseClicked(MouseEvent e) {
                txtS3.setText(Global_data.uc.get(z));
                //题目
                txtS4.setText(Global_data.uv.get(Global_data.uc.get(z)));
                //答案
                correct--;
                //每次递减
                if (Global_data.uv.get(Global_data.uc.get(z)).equals(txtS1.getText())) {
                    remaining++;
                    if (Global_data.ckb) {
                        try {
        ...
        setTitle("用户" + Global_data.user + "——单词训练（英译中）");
        //设置窗体标题
        setResizable(false);
        //不可调节窗口大小
        setVisible(true);
